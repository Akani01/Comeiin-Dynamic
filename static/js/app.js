const supplies = (window.COMEIIN_PRODUCTS || []).map((product) => ({
  ...product,
  image: product.image || `${window.CW_STATIC || '/static/'}assets/products/${product.id}-generated-v2.webp`,
...
// Catalogue base URL — Django route for the products page
...
document.addEventListener('click', (e) => {
  ...
  // Product detail → open modal
  if (b.dataset.detail) {
    e.preventDefault();
    const p = supplies.find((p) => p.id === b.dataset.detail);
    if (!p) return;
    $('#detail-body').innerHTML = detailsMarkup(p);
    detail.showModal();
    return;
  }
...
document.addEventListener('click', (e) => {
  ...
  if (browse) {
    e.preventDefault();
    if (!$('#catalogue')) {
      // We're on a page that has no catalogue section → navigate
      location.href = `${CATALOGUE_URL}?category=${encodeURIComponent(browse.dataset.browse)}#catalogue`;
      return;
    }
    // We're on the catalogue page → filter in place
...
if (currentProduct) {
  ...
  $('#product-view').innerHTML =
    '<a class="back-link" href="#" onclick="history.back();return false;">← Back to our range</a>' +
